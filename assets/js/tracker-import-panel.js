...
import { loadoutEntry, loadoutName } from './loadout.js?v=68';
import { saveLoadoutCatalog } from './loadout-catalog.js?v=68';
import { perkName } from './perks.js?v=68';
import { facedKillersLabel, gameModeLabel, labelFor } from './data.js?v=68';
import { escapeHtml, fmtDate, fmtNumber, toast } from './utils.js?v=68';
import { attachBuilds, markDuplicates, parseMatchHistory } from './tracker-import.js?v=68';
...
async function mountBookmarklet() {
  ...
  try {
    const res = await fetch('assets/js/tracker-bookmarklet.js?v=68');
...
async function analyse() {
  ...
  try {
    ...
    /*
      Namen und Symbole aus dem Tracker gelten sofort. Gesichert werden sie hier
      und nicht erst beim Übernehmen: Wer die Historie nur einfügt, um die
      Beschriftung aufzufrischen, hat lauter Dubletten vor sich – dann ist nichts
      ausgewählt, der Übernehmen-Knopf bleibt aus, und das Gelernte ginge verloren.
      Mit den Matches hat der Katalog ohnehin nichts zu tun.
    */
    const fresh = parsed.catalog.filter((part) => !loadoutEntry(part.kind, part.id)).length;
    await saveLoadoutCatalog(parsed.catalog, currentUser.id);
